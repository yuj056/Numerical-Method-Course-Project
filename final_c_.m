...
for i=1:length(t)-1;
    fscn2(:,i+1)=thomas1(a,b,c,fscn2(:,i),q2,beta,gamma,dt);
 T2(:,:,i+1) = zeros(size(T2(:,:,1)));
    
    for j = 1:Ny;
        for m = 1:Nx;
            
            T2(j,m,i+1) = sin(2*pi*k*x(m)/lx)*fscn2(j,i+1);
...
for i=1:length(t)-1;
    fscn3(:,i+1)=thomas1(a,b,c,fscn3(:,i),q3,beta,gamma,dt);
T3(:,:,i+1) = zeros(size(T3(:,:,1)));
    
    for j = 1:Ny;
        for m = 1:Nx;
            
            T3(j,m,i+1) = sin(2*pi*k*x(m)/lx)*fscn3(j,i+1);
